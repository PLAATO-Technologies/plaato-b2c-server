...
public class DeviceWorkflowTest extends SingleServerInstancePerTest {
    ...
    public void testSendHardwareCommandToAppFromMultipleDevices() throws Exception {
        ...
        clientPair.hardwareClient.send("hardware vw 90 101");
        verify(clientPair.appClient.responseMock, timeout(500)).channelRead(any(), eq(new HardwareMessage(1, b("1-0 vw 90 101"))));

        hardClient2.send("hardware vw 90 100");
        verify(clientPair.appClient.responseMock, timeout(500)).channelRead(any(), eq(new HardwareMessage(2, b("1-1 vw 90 100"))));
    }
